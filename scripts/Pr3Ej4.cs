...
public class Pr3Ej4 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.W) || Input.GetKey(KeyCode.S)) {
            sphere_tf.Translate(Vector3.up * Input.GetAxis("Vertical") * speed);
        }

        if (Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.D)) {
            sphere_tf.Translate(Vector3.right * Input.GetAxis("Horizontal") * speed);
        }
